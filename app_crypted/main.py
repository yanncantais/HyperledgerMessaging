...
class Connected(Screen):         
    ...
    def contact(self, contact):
        ...
        loop.run_until_complete(store_did(app.username, app.login, contact, TheirDID, TheirVerkey))
        ...
        #     store_did(app.username, app.login, contact, TheirDID, TheirVerkey)           
                        ...
                        if isinstance(button, Button):
                            if button.name != "profile":
                                if button.connection_id == connection:    
                                    if ":" in app.Contacts[connection]:
                                        real_name = app.Contacts[connection].split(":")[0]
                                    else:
                                        real_name = app.Contacts[connection]
                                    button.text = real_name + " ("+str(len(app.NewMessages[connection]))+")"
    ...
    def on_enter(self):
        ...
        for item in Contacts:
            ...
            if ":" in contact:
                ...
                their_did = contact.split(":")[1]
            else:
                name = contact
            if connection_id in app.NewMessages:
                counter = len(app.NewMessages[connection_id])
                if counter > 0:
                    text = name+" ("+str(counter)+")"
                else:
                    text = name
            else:
                text = name
            button = Button(text=text, on_press=lambda btn, item=(name, connection_id): self.button_pressed(*item), size_hint=(1, 0.3), name="contact")
            button.font_size = 20
...
async def ask_verkey(wallet_handle, user, contact):
    path = user+"/their_dids.did"
    if not os.path.isfile(path):
        f = open(path, "w+")
    f = open(path, "r")
...
class ChatPage(Screen):
        ...
        while self.manager.current == 'chat_page':     
            ...
            for message in Messages:
                ...
                if Encrypted:
                    ...
                    content = loop.run_until_complete(decrypt_message(app.login, content))  
    ...
    def on_enter(self, **kwargs):
        ...
        thread.start()
        loop = asyncio.get_event_loop()
        verkey = loop.run_until_complete(ask_verkey(app.login, app.username, app.contact))
        if verkey is None:
            self.send(message=ask_verkey_label, encrypted=False)
    ...
    def send(self, message, encrypted = True):
        ...
        if encrypted:        
            verkey = None
            path = app.username+"/their_dids.did"
            f = open(path, "r")
            DIDs = f.read().split("\n")           
            loop = asyncio.get_event_loop()
            verkey, encrypted_message = loop.run_until_complete(send_encrypt(app.username, app.login, DIDs, message, app.contact))
            
            if verkey == None:
                print("verkey not found")
                exit(1)
        else:
            encrypted_message = message
        print("message", message, encrypted_message, "will be sent")
        if message != ask_verkey_label:
        ...
        response = requests.post(url, json = {"content": encrypted_message})
        if message == ask_verkey_label:
            return ""
